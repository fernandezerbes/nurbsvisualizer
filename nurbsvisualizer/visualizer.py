...
class CurveVisualizer:
    ...
    def __init__(self, curve):
        ...
        self.cursor = Cursor(
            self.ax_basis, horizOn=False, vertOn=True, linewidth=0.5, color="grey"
        )
        ...
        self.key_press_event = self.fig.canvas.mpl_connect(
            "key_press_event", self.on_key_press
        )
        self.motion_notify_event_basis = self.fig.canvas.mpl_connect(
            "motion_notify_event", self.on_motion
        )
        self.pick_event = self.fig.canvas.mpl_connect("pick_event", self.on_pick)
        if self.is_3D:
            self.motion_notify_event_curve = self.fig.canvas.mpl_connect(
                "motion_notify_event", self.update_control_point_label_position
            )
    ...
    def basis_plot_setup(self):
        ...
        self.ax_basis.tick_params(
            axis="x", which="major", length=10, color="k", width=2, direction="inout"
        )
    ...
    def plot_basis(self):
        ...
        for i, basis in enumerate(self.curve.basis_evals):
            ...
            basis_label = (
                "$R_{" + str(i) + "," + str(self.curve.polynomial_degree) + "}(\\xi)$"
            )

            # Plot the current basis
            (art,) = self.ax_basis.plot(
                self.curve.samples, basis, label=basis_label, picker=5
            )
    ...
    def plot_curve(self):
        """Plot the curve."""
        if self.is_3D:
            (self.curve_art,) = self.ax_curve.plot(
                self.curve.curve_evals[0],
    ...
    def plot_control_polygon(self):
        ...
        if self.is_3D:
            (self.control_polygon_art,) = self.ax_curve.plot(
                self.curve.control_points[0],
            ...
            (self.control_polygon_marker_art,) = self.ax_curve.plot(
                self.curve.control_points[0],
                self.curve.control_points[1],
                self.curve.control_points[2],
                ls="none",
    ...
    def plot_knot_points(self):
        ...
        indices = [
            get_index_to_nearest(self.curve.samples, knot) for knot in knots[1:-1]
        ]
        ...
        if self.is_3D:
            ...
            (self.knot_points_art,) = self.ax_curve.plot(
                x, y, z, ls="none", marker="s", mec="k", mfc="w", ms=5
            )

        else:
            (self.knot_points_art,) = self.ax_curve.plot(
                x, y, ls="none", marker="s", mec="k", mfc="w", ms=5
            )
    ...
    def plot_knot_lines(self):
        ...
        for knot in knots:
            ...
            y = [0, 1]
            (art,) = self.ax_basis.plot(
                x, y, linestyle="--", color="gray", linewidth=0.5
            )
    ...
    def on_key_press(self, event):
        ...
        if event.key == "c":
            ...
            self.control_polygon_art.set_visible(
                not self.control_polygon_art.get_visible()
            )
    ...
    def on_motion(self, event):
        ...
        if self.ax_basis == event.inaxes:
            ...
            if self.curve.knot_vector[0] <= event.xdata <= self.curve.knot_vector[-1]:
                ...
                nearest_point_idx = get_index_to_nearest(
                    self.curve.samples, event.xdata
                )
    ...
    def update_picked_basis_index(self, event):
        ...
        if (
            self.picked_basis_index is None
            or self.picked_basis_index != new_picked_basis_index
        ):
    ...
    def update_control_point_label(self):
        ...
        if self.picked_basis_index is not None:
            ...
            if self.is_3D:
                ...
                self.control_point_label = self.ax_curve.annotate(
                    label,
                    xy=(x2, y2),
                    xytext=(-20, 20),
    ...
    def get_current_basis_label(self):
        ...
        return (
            "$R_{"
            + str(self.picked_basis_index + 1)
            + ","
            + str(self.curve.polynomial_degree)
            + "}(\\xi)$"
        )
...
class SurfaceVisualizer:
    ...
    def __init__(self, surface):
        ...
        self.motion_notify_event = self.fig.canvas.mpl_connect(
            "motion_notify_event", self.update_control_point_label_position
        )
    ...
    def plot_basis(self):
        ...
        xi_grid, eta_grid = np.meshgrid(
            self.surface.samples[0], self.surface.samples[1]
        )
        self.basis_function_art = self.ax_basis.plot_surface(
            xi_grid,
            eta_grid,
            self.surface.basis_evals[self.current_basis_i_idx][
                self.current_basis_j_idx
            ],
            antialiased=False,
        )
    ...
    def get_current_basis_label(self):
        ...
        return (
            "$R_{"
            + str(self.current_basis_i_idx + 1)
            + str(self.current_basis_j_idx + 1)
            + "}(\\xi, \\eta)$"
        )
    ...
    def plot_surface(self):
        """Plot the surface."""
        self.surface_art = self.ax_surface.plot_surface(
            self.surface.surface_evals[0],
    ...
    def update_control_point_label(self, direction=None):
        ...
        self.control_point_label = self.ax_surface.annotate(
            label,
            xy=(x2, y2),
            xytext=(-20, 20),
            textcoords="offset points",
    ...
    def get_current_control_point_coords(self):
        ...
        numpy.ndarray
            The x, y, z coordinates of the current control point.
        """
        return self.surface.control_points[
            :, self.current_basis_i_idx, self.current_basis_j_idx
        ]
    ...
    def plot_control_polygon(self):
        ...
        for row in range(self.surface.control_points_count[0]):
            (art,) = self.ax_surface.plot(
                self.surface.control_points[0, row, :],
        ...
        for column in range(self.surface.control_points_count[1]):
            (art,) = self.ax_surface.plot(
                self.surface.control_points[0, :, column],
